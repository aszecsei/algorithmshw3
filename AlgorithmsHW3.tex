...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{LargestSubsetNonIntersecting}{$set[1..n]$}
			...
			\State $set[0] \gets (-\infty, -\infty)$
			\For{$i \gets 1, n$} \Comment Base cases
		...
		\Function{MostFunParty}{$root$}
			...
			\While{$i \leq$ the length of $queue$}
				...
				\If{$current.left$ exists}
					\State push $current.left$ onto $queue$
				\EndIf
				\If{$current.right$ exists}
					\State push $current.right$ onto $queue$
				\EndIf
				\State $i \gets i + 1$
			\EndWhile
			\State reverse $queue$
			\State $n \gets$ the length of $queue$
			\For{$i \gets 1, n$}
				\State $current \gets queue[i]$
				\State $maxA \gets current.fun$
				\State $listA \gets [current]$ \Comment if the root is included in the party
				\State $maxB \gets 0$
				\State $listB \gets []$ \Comment if the root is not included in the party
...
Next, we can go through the list and define sub-problems.\\
