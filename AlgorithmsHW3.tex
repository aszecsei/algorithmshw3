...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\State{$memCards \gets$ empty hash map}
		\Function{Elmo}{$cards[1..n]$}
			\If{$n = 0$}
				\State \Return $0$
			\ElsIf{$n = 1$}
				\State \Return $cards[1]$
			\ElsIf{$memCards$ contains $cards$}
				\State \Return $memCards[cards]$
			\Else
				\State $L \gets cards[2..n]$ \Comment Try taking the first card
				\If{$cards[2] > cards[n]$} \Comment Remove Elmo's card
					\State remove the first element from $L$
				\Else
					\State remove the last element from $L$
				\EndIf
				\State $l \gets cards[1] +$ \Call{Elmo}{$L$}
				\State $R \gets cards[1..n-1]$ \Comment Try taking the last card
				\If{$cards[1] > cards[n-1]$} \Comment Remove Elmo's card
					\State remove the first element from $R$
				\Else
					\State remove the last element from $R$
				\EndIf
				\State $r \gets cards[n] +$ \Call{Elmo}{$R$}
				\State $x \gets$ the maximum of $l$ and $r$
				\State Store $x$ in $memCards[cards]$
				\State \Return $x$
			\EndIf
		\EndFunction
	\end{algorithmic}
